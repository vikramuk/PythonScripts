...
 
#Husain modifications for ULS1
datadictULS1 = [dict() for i in range(10)]
 
 
#
 
...
        self._dev_uls1 = None
        self.ep_uls1 = None
        self.ep_read_uls1 = None
        self._varListULS1 = [dict() for i in range(10)]
        self._ULS1_columnCount = 1
        self._rowCount = 3
        
        
...
        #ULS1
        # find our device
        self._dev_uls1 = usb.core.find(idVendor=0x1901, idProduct=0x0061)
 
           
        #print dev
        # was it found?
        if self._dev_uls1 is None:
            pass
        else:
            
            # set the active configuration. With no arguments, the first configuration will be the active one
            self._dev_uls1.set_configuration()
            # get an endpoint instance
            cfg_uls1 = self._dev_uls1.get_active_configuration()
            intf_uls1 = cfg_uls1[(0,0)]#usb.util.find_descriptor(cfg, bInterfaceNumber=2)
            self.ep_uls1 = usb.util.find_descriptor(
                intf_uls1,
                # match the first OUT endpoint
                custom_match=lambda e: \
                usb.util.endpoint_direction(e.bEndpointAddress) == \
                usb.util.ENDPOINT_OUT)
            assert self.ep_uls1 is not None
 
            self.ep_read_uls1 = usb.util.find_descriptor(
                intf_uls1,
                # match the first IN endpoint
                custom_match=lambda e: \
                usb.util.endpoint_direction(e.bEndpointAddress) == \
                usb.util.ENDPOINT_IN)
 
            assert self.ep_read_uls1 is not None
            # Send Start up Request Packet
            datasend_uls1 =  '\x30\x5E\xED\xBA\xDE\x00\x12\x00\x01\x1F\x5A\x00\x46\x00\x10\x00\x04\x00\x08\xCD\x49\xBE\x53\x89\x99\x2D\x2C\x20\xB0\x7F\x65\xC5\x96\x9D\x7A\x00\x0A\x00\x00\x00\x02\x00\x00\x00\x07\x00\x00\xE8\x1C'
            self.ep_uls1.write(datasend_uls1)
 
            while True:
                try:
                    # Get the Start up Response Packet.
                    data_uls1 = self.ep_read_uls1.read(64)
                    #print data
                    # print ' '.join([hex(Byte)[2:] for Byte in data])
                    break
                except usb.core.USBError as e:
                    sys.exit("Could not read from endpoint: %s" % str(e))
            # Send Start up Complete Packet
            datawrite_uls1= '\x18\x5E\xED\xBA\xDE\x00\x06\x00\x02\xB0\x9A\x00\x46\x00\x04\x00\x0E\x00\x00\x00\x02\x00\x00\x75\x26'
            self.ep_uls1.write(datawrite_uls1)
            #Get Startup complete Response Packet.
            data_uls1 = self.ep_read_uls1.read(64)
            threading.Thread(target=self.mycallback_uls1).start()
            time.sleep(1)
 
 
       
...
 
       #ULS1         
    def mycallback_uls1(self):
        
        Label(self, text="ULS1", padx=82, pady=20, relief=RIDGE, bg="MediumSeaGreen").grid(row=3,rowspan=2)
        # Old CRC
        old_crc = [0,0]
        newPacket = []
        audiobuffer = []
...
        while True:
            try:
                data = self.ep_read_uls1.read(self.ep_read_uls1.wMaxPacketSize, timeout=100000)  #  40000000
...
                            old_crc = newPacket[-2:]
                            pdata = ' '.join([format(Byte, '02x') for Byte in newPacket])
                            self.CheckAndPrintULS1(bytearray.fromhex(pdata))    
                    newPacket = data[11:]
                else:
                    newPacket.extend(data[1:])
                #print ' '.join([format(Byte, '02x') for Byte in data])
...
                    child_data = parent_data[ci.get():ci.forward(child_length * WORD_BYTES)]
                    #print "Child Tag #: ", '0x{0:04x}'.format(child_id) , 'Child Data #: ' ,' '.join(['{0:02x}'.format(ch) for ch in child_data])
                    if child_id not in datadictULS1[parent_id%10]:
                        datadictULS1[parent_id%10][child_id] = child_data
                        local = ' '.join(['{0:02x}'.format(ch) for ch in child_data])
                        self._varListULS1[parent_id%10][child_id] = StringVar()
                        self._varListULS1[parent_id%10][child_id].set(local)
                        Label(self, text="Child Tag "+str(child_id), padx=32, pady=5,relief=RIDGE).grid(row=self._rowCount, padx=5, column=self._ULS1_columnCount)
                        Entry(self, textvariable=self._varListULS1[parent_id%10][child_id], justify=CENTER, state = "readonly",readonlybackground="white").grid(row=self._rowCount+1, column=self._ULS1_columnCount, padx=7, ipady=4)
                        self._ULS1_columnCount += 1
                        if (self._ULS1_columnCount == 8):
                            self._rowCount += 2
                            self._ULS1_columnCount = 1
                        #print "Child Tag #: ", '0x{0:04x}'.format(child_id) , 'Child Data #: ' ,' '.join(['{0:02x}'.format(ch) for ch in child_data])
                    elif datadictULS1[parent_id%10][child_id]!=child_data:
                        datadictULS1[parent_id%10][child_id] = child_data
                        local = ' '.join(['{0:02x}'.format(ch) for ch in child_data])
                        self._varListULS1[parent_id%10][child_id].set(local)
                        #print "Child Tag #: ", '0x{0:04x}'.format(child_id) , 'Child Data #: ' ,' '.join(['{0:02x}'.format(ch) for ch in child_data])
